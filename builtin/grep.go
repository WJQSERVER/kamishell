...
func Grep(args []string, env Environment, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	...
	m := RegisterMeta("grep")
	opts := &grepOptions{}
	BoolFlagVar(fs, m, &opts.ignoreCase, "ignore-case", "i", false, "ignore case distinctions")
	BoolFlagVar(fs, m, &opts.lineNumber, "line-number", "n", false, "print line number")
	BoolFlagVar(fs, m, &opts.invertMatch, "invert-match", "v", false, "invert match")
	BoolFlagVar(fs, m, &opts.wordRegexp, "word-regexp", "w", false, "match whole words")
	BoolFlagVar(fs, m, &opts.lineRegexp, "line-regexp", "x", false, "match whole lines")
	BoolFlagVar(fs, m, &opts.count, "count", "c", false, "print count of matching lines")
	BoolFlagVar(fs, m, &opts.filesMatch, "files-with-matches", "l", false, "list filenames with matches")
